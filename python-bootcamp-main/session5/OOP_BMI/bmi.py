...
class Person:
	def __init__(self, id, fname, lname,  email):
		...
		self.lname = lname
		self.age = 0
		self.weight = 0 #dont want updated w, h, age, so not included in args
	...
	def calc_class(self ):
		...
		self.age = self.find_age() #find age is not defined because when in def calc class, we will call all fns within ur class so use self for all find_age
